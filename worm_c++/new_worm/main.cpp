...

{
	...
	if (net_info.size() == 2)
	{
		...
		for (const auto& ip_ws : range) {
			bool host_bool = is_host_alive(ip_ws); // 0 <-> is_host_alive(ip_ws); // помен€ть (раскомментировать)
	...
	for (wstring& ip : alive_ip) { // alive_ip <-> ip_vector // помен€ть
		wcout << L"провер€ем " << ip << endl; // удалить
